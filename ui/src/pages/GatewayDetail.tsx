...
export default function GatewayDetailPage() {
  ...
  if (!data) return <div className="text-center py-10 text-a-danger text-sm">Gateway not found</div>;

  return (
    <div>
      <button onClick={() => navigate('/gateways')} className="inline-flex items-center gap-1 text-xs text-a-muted hover:text-a-fg mb-3 bg-transparent border-none cursor-pointer p-0">← Gateways</button>
      <PageHeader title={data.name} subtitle={`gateway_id: ${data.gateway_id}`} helpKey="gateways" />

      <div className="grid grid-cols-2 gap-4 mb-4">
        <Card title="基本信息">
          <MetaRow label="Gateway ID" value={data.gateway_id} mono color="text-a-accent" />
          <MetaRow label="Name" value={data.name} />
          <MetaRow label="Node" value={
          ...
          <MetaRow label="Type" value={<StatusBadge status={data.type === 'local' ? 'local_gateway' : data.type === 'private' ? 'private_gateway' : 'public_gateway'} />} />
          <MetaRow label="Provider" value={data.provider} />
          <MetaRow label="Bind" value={`${data.bind_addr}:${data.port}`} mono />
          <MetaRow label="Scheme" value={data.scheme} mono />
        </Card>

        <Card title="访问控制">
          <MetaRow label="Public Accessible" value={data.public_accessible ? '✓ 是' : '✗ 否'} color={data.public_accessible ? 'text-[#e8b830]' : ''} />
          <MetaRow label="Private Accessible" value={data.private_accessible ? '✓ 是' : '✗ 否'} color={data.private_accessible ? 'text-[#4cd964]' : ''} />
          <MetaRow label="Enabled" value={data.enabled ? '✓' : '✗'} />
          <MetaRow label="Priority" value={data.priority} mono />
          <MetaRow label="Status" value={<StatusBadge status={data.status} />} />
          <MetaRow label="Routes Served" value={data.routes_served} mono />
        </Card>
      </div>
      ...
      <MetaRow label="Last Verified" value={fmtRel(data.last_verified_at)} mono className="mt-2" />
    </div>
  );
}

...
import { PageHeader, Card, QueryGuard, StatusBadge, Btn, Timestamp } from '@/components/shared';
import { fetchNodes } from '@/lib/api-bridge';

export default function Updates() {
  const { items: nodes, isLoading, error, refetch } = useApiList<any>(['updates-nodes'], () => fetchNodes());
  const outdated = nodes.filter((n: any) => n.sync_status === 'outdated' || (n.desired_revision && n.applied_revision && n.desired_revision !== n.applied_revision));

  return (
    <div className="p-6 space-y-6">
      <PageHeader title="更新管理" subtitle={outdated.length > 0 ? `${outdated.length} 个节点需要更新` : '所有节点已是最新'} />
      <QueryGuard items={nodes} isLoading={isLoading} error={error} refetch={refetch} emptyMessage="暂无节点数据">
        {() => (
          <>
            {outdated.length > 0 && (
              <Card title="待更新节点">

...
export default function GatewayLinks() {
  ...
  const handleRotate = async () => {
    if (!selected) return;
    const risk = evaluateRisk('rotate_gateway_link', 'gateway_link', selected.id, selected.id);
    if (risk.tier === 'high') {
      if (!confirm(`确认轮换链路 ${selected.id} 的密钥？旧密钥将立即失效。`)) return;
    }
    setRotating(true);
    try {
      await gatewayLinkApi.rotate(selected.id);
      toast('密钥已轮换');
  ...
  const handleDelete = async () => {
    if (!selected) return;
    if (!confirm(`确认删除链路 ${selected.id}？依赖此链路的路由将无法跨节点转发。`)) return;
    setDeleting(true);
    try {
      await gatewayLinkApi.delete(selected.id);
      toast('链路已删除');
      queryClient.invalidateQueries({ queryKey: ['gateway-links'] });
      setDrawerOpen(false);
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="space-y-1">
        {links.map((l: any) => (
          <div key={l.id}
            ...
            <span className="font-medium text-a-fg w-28 text-right truncate">{l.name}</span>
            <div className="flex items-center gap-1.5">
              <div className={cn('w-12 h-px', l.status === 'active' ? 'bg-[#4cd964]/60' : l.status === 'failed' ? 'bg-[#ff5c72]/60' : 'bg-a-border')} />
              <StatusBadge status={l.status} />
              <div className={cn('w-12 h-px', l.status === 'active' ? 'bg-[#4cd964]/60' : l.status === 'failed' ? 'bg-[#ff5c72]/60' : 'bg-a-border')} />
            </div>
            <span className="font-medium text-a-fg w-28 truncate">{l.host + (l.port ? ':' + l.port : '')}</span>
      ...
      <Drawer
        ...
        title={selected ? `${selected.name || selected.id} → ${selected.host}:${selected.port || 80}` : '链路详情'}
        ...
        {selected && (
          <div className="space-y-4">
            <Card title="链路信息">
              ...
              <MetaRow label="源节点" value={selected.name || selected.id} />
              <MetaRow label="主机地址" value={selected.host + (selected.port ? ':' + selected.port : '')} />
              <MetaRow label="内网IP" value={selected.private_ip || '—'} />

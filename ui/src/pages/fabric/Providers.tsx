...
function TransparentProxyPanel() {
  ...
  const forwardTargets = status?.forward_targets || [];
  ...
  return (
    <div className="space-y-5">
      {/* Diagnosis */}
      <Card title="可用性诊断" subtitle={statusLoading ? '检查中...' : allPassed ? '透明代理已就绪' : `${passedCount}/${checks.length} 条件满足`}>
        {statusLoading ? (
          <div className="text-sm text-a-muted py-6 text-center">检查中...</div>
        ) : checks.length === 0 ? (
          <div className="text-sm text-a-muted py-6 text-center">无法获取诊断数据</div>
        ) : (
          <div className="space-y-2">
            {checks.map((c: any, i: number) => (
              <div key={i} className={cn(
                'flex items-center gap-3 px-3 py-2.5 rounded-a-sm border text-xs',
                c.passed ? 'bg-[#4cd964]/5 border-[#4cd964]/15' : 'bg-[#ff5c72]/5 border-[#ff5c72]/15',
              )}>
                <span className={cn('font-mono text-sm shrink-0', c.passed ? 'text-[#4cd964]' : 'text-[#ff5c72]')}>
                  {c.passed ? '✓' : '✗'}
                </span>
      ...
      <Card title={`iptables 规则 (${rules.length})`} subtitle="目标 IP:端口 → 本地代理端口">
        {rulesLoading ? (
          <div className="text-sm text-a-muted py-8 text-center">加载中...</div>
        ) : rules.length === 0 ? (
              ...
              <tbody>
                {rules.map((rule: any) => (
                  <tr key={rule.id} className="border-b border-a-border/50 hover:bg-a-border/10">
                    <td className="py-2 px-3 font-mono">{rule.original_ip}:{rule.original_port}</td>
                    <td className="py-2 px-3 font-mono text-a-muted">:{rule.local_proxy_port}</td>
                    <td className="py-2 px-3"><StatusBadge status={rule.active ? 'active' : 'disabled'} /></td>
                    <td className="py-2 px-3 font-mono text-[11px]">↓{rule.bytes_in || 0} ↑{rule.bytes_out || 0}</td>
                    <td className="py-2 px-3">
                      <button onClick={() => deleteMutation.mutate(rule.id)}
                        className="text-[10px] px-2 py-0.5 rounded border border-[#ff5c72]/30 text-[#ff5c72] hover:bg-[#ff5c72]/10 transition-colors cursor-pointer">删除</button>
                    </td>
                  </tr>
                ))}
              </tbody>

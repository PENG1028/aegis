...
func TestIsCurrentNodeAddress(t *testing.T) {
	...
	if !IsCurrentNodeAddress("43.160.211.232", selfIPs) {
		t.Errorf("43.160.211.232 should be current node address")
	}
...
func TestRiskNoBypassWithGatewayLink(t *testing.T) {
	svc := NewService(Dependencies{})
	risks := svc.GetPlannerWarnings("test.local", "43.159.34.11:80", "gw_link_123")
	for _, r := range risks {
		if r.Code == RiskGatewayLinkBypass {

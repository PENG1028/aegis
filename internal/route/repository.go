...
func (r *Repository) Create(rt *Route) error {
	...
	_, err := r.DB.Exec(
		`INSERT INTO routes (id, domain, path_prefix, strip_prefix, service_id, tls_enabled, status, maintenance_enabled, maintenance_message, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (r *Repository) FindAll() ([]Route, error) {
	rows, err := r.DB.Query(
		`SELECT id, domain, path_prefix, strip_prefix, service_id, tls_enabled, status, maintenance_enabled, maintenance_message, created_at, updated_at
		 FROM routes ORDER BY domain, LENGTH(path_prefix) DESC`)
	if err != nil {
...
func (r *Repository) FindByID(id string) (*Route, error) {
	...
	).Scan(&rt.ID, &rt.Domain, &pathPrefix, &stripVal, &rt.ServiceID, &tlsVal, &rt.Status, &maintVal, &maintMsg, &createdAt, &updatedAt)
	if err != nil {
...
func (r *Repository) FindByDomain(domain string) (*Route, error) {
	...
	var tlsVal, maintVal, stripVal int
	var maintMsg sql.NullString
	// For backward compat: find domain-only route first
	err := r.DB.QueryRow(
		`SELECT id, domain, path_prefix, strip_prefix, service_id, tls_enabled, status, maintenance_enabled, maintenance_message, created_at, updated_at
		 FROM routes WHERE domain = ? AND (path_prefix IS NULL OR path_prefix = '') LIMIT 1`, domain,
	).Scan(&rt.ID, &rt.Domain, &pathPrefix, &stripVal, &rt.ServiceID, &tlsVal, &rt.Status, &maintVal, &maintMsg, &createdAt, &updatedAt)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
...
func (r *Repository) FindByDomainAndPath(domain, pathPrefix string) (*Route, error) {
	...
	if pathPrefix == "" {
		...
		row = r.DB.QueryRow(
			`SELECT id, domain, path_prefix, strip_prefix, service_id, tls_enabled, status, maintenance_enabled, maintenance_message, created_at, updated_at
			 FROM routes WHERE domain = ? AND path_prefix = ?`, domain, pathPrefix)
	}
	err := row.Scan(&rt.ID, &rt.Domain, &dbPathPrefix, &stripVal, &rt.ServiceID, &tlsVal, &rt.Status, &maintVal, &maintMsg, &createdAt, &updatedAt)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (r *Repository) FindActive() ([]Route, error) {
	rows, err := r.DB.Query(
		`SELECT id, domain, path_prefix, strip_prefix, service_id, tls_enabled, status, maintenance_enabled, maintenance_message, created_at, updated_at
		 FROM routes WHERE status = 'active' ORDER BY domain, LENGTH(COALESCE(path_prefix,'')) DESC`)
	if err != nil {
...
func (r *Repository) Update(rt *Route) error {
	...
	_, err := r.DB.Exec(
		`UPDATE routes SET domain=?, path_prefix=?, strip_prefix=?, service_id=?, tls_enabled=?, status=?, maintenance_enabled=?, maintenance_message=?, updated_at=? WHERE id=?`,
...
func scanRoutes(rows *sql.Rows) ([]Route, error) {
	...
	for rows.Next() {
		...
		var tlsVal, maintVal, stripVal int
		var maintMsg sql.NullString
		if err := rows.Scan(&rt.ID, &rt.Domain, &pathPrefix, &stripVal, &rt.ServiceID, &tlsVal, &rt.Status, &maintVal, &maintMsg, &createdAt, &updatedAt); err != nil {

...
func TestNewTrustedGateway(t *testing.T) {
	gw := NewTrustedGateway("server-b", "43.159.34.11", "10.3.0.11", 443, "test-secret", TypeUpstream, true)

	if gw.Name != "server-b" {
		t.Errorf("expected Name=server-b, got %s", gw.Name)
	}
	if gw.Host != "43.159.34.11" {
		t.Errorf("expected Host=43.159.34.11, got %s", gw.Host)
	}

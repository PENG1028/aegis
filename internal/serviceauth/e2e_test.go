...
func runTestMigrations(t *testing.T, db *sql.DB) {
	...
	for i, m := range []string{
		`CREATE TABLE IF NOT EXISTS svc_auth_services (id TEXT PRIMARY KEY, name TEXT NOT NULL, host TEXT, port INTEGER DEFAULT 0, node_host TEXT DEFAULT '', apis_json TEXT NOT NULL DEFAULT '[]', public_key TEXT NOT NULL DEFAULT '', instance_id TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'active', last_seen TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL DEFAULT '', updated_at TEXT NOT NULL DEFAULT '')`,

...
func runTestMigrations(t *testing.T, db *sql.DB) {
	...
	for i, m := range []string{
		`CREATE TABLE IF NOT EXISTS svc_auth_services (id TEXT PRIMARY KEY, name TEXT NOT NULL, host TEXT, port INTEGER DEFAULT 0, listen_port INTEGER DEFAULT 0, node_host TEXT DEFAULT '', apis_json TEXT NOT NULL DEFAULT '[]', public_key TEXT NOT NULL DEFAULT '', instance_id TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'active', last_seen TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL DEFAULT '', updated_at TEXT NOT NULL DEFAULT '')`,
		`CREATE INDEX IF NOT EXISTS idx_svc_auth_name ON svc_auth_services(name)`,
		`CREATE TABLE IF NOT EXISTS svc_auth_call_logs (id TEXT PRIMARY KEY, caller_service TEXT NOT NULL DEFAULT '', target_service TEXT NOT NULL DEFAULT '', target_api TEXT NOT NULL DEFAULT '', caller_host TEXT DEFAULT '', target_host TEXT NOT NULL DEFAULT '', allowed INTEGER NOT NULL DEFAULT 1, latency_ms INTEGER NOT NULL DEFAULT 0, error_msg TEXT NOT NULL DEFAULT '', called_at TEXT NOT NULL DEFAULT '')`,

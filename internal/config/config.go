...
// PanelCaddyfile returns a Caddyfile that reverse-proxies the Aegis panel
// (API + embedded UI) on port 80, forwarding to the internal listen address.
//
// This is generated by `aegis bootstrap --production` so the panel is
// publicly accessible without manual Caddy configuration.
func (c *Config) PanelCaddyfile() string {
	return `# Aegis Control Panel — auto-generated by bootstrap
# Serves the Aegis web panel (API + UI) via reverse proxy.
# User routes are managed through the Aegis UI and rendered separately.

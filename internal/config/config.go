...
// PanelCaddyfile returns a Caddyfile snippet that reverse-proxies the Aegis
// panel (API + embedded UI) on port 80, forwarding to the internal listen address.
//
// Generated by `aegis bootstrap --production`.
//
// IMPORTANT: This snippet only handles the Aegis UI/API path (/api/* and /).
// User routes are managed by Aegis itself and rendered to a separate Caddyfile.
// Both configs can coexist — Caddy merges them.
func (c *Config) PanelCaddyfile() string {
	return `# Aegis Control Panel — auto-generated by aegis bootstrap
# Reverse-proxies the Aegis web panel (API + UI) on port 80.
#
# This file ONLY handles the Aegis panel. User-defined routes are managed
# through the Aegis UI and rendered to a separate configuration file.
# Both files coexist safely — Caddy merges multiple configs automatically.
#
# To disable: remove or rename this file, then reload Caddy.
#   sudo mv /etc/caddy/aegis-panel.conf /etc/caddy/aegis-panel.conf.disabled

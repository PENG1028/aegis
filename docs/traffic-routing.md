...
```
两台机器: A (<SERVER_A_IP>, 内网 10.0.0.3, network=vpc-tokyo)
         B (<SERVER_B_IP>,  内网 10.0.0.5, network=vpc-tokyo)
```
...
外部 curl https://api.example.com/resource
  │
  ▼ DNS 解析 → <SERVER_A_IP> (Machine A 公网 IP)
  │
  ▼ connect(<SERVER_A_IP>, 80) → Machine A Caddy
  │
...
外部 curl https://api.example.com (TLS)
  │
  ▼ connect(<SERVER_A_IP>, 443) → HAProxy (EdgeMux) 或 Caddy HTTPS
  │
...
```
Machine A 进程 → connect(<SERVER_A_IP>, 8080)
  │                              ↑ 自己的公网 IP
  │
  ▼ iptables OUTPUT DNAT: -d <SERVER_A_IP> --dport 8080 → 127.0.0.1:18100
  │

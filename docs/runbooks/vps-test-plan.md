...
```
Server A (面板+网关): ssh ubuntu@43.160.211.232
Server B (后端服务):   ssh ubuntu@43.159.34.11
...
```bash
ssh ubuntu@43.160.211.232 "curl -s http://127.0.0.1:7380/api/healthz"
# → "OK"

ssh ubuntu@43.160.211.232 "curl -s http://127.0.0.1:7380/api/admin/v1/service-auth/services"
# → {"services": []}
```

### 1.3 安装基础设施依赖
```bash
ssh ubuntu@43.160.211.232 "sudo apt install -y certbot iptables dnsmasq"
ssh ubuntu@43.159.34.11 "sudo apt install -y certbot iptables dnsmasq"
...
# Server B (跨节点)
go run . -name=monitor-svc -port=8084 -aegis=http://43.160.211.232:7380 &
```
...
# 每个服务不同的 Ed25519 公钥
ssh ubuntu@43.160.211.232 "sqlite3 /var/lib/aegis/aegis.db 'SELECT name, substr(public_key,1,20) FROM svc_auth_services'"
...
curl -s http://43.159.34.11:7380/api/admin/v1/service-auth/services
```

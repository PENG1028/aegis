...
| Machine | IP | Role |
|---------|-----|------|
| Server A | <SERVER_A_IP> | Control plane (port 9000), node-a, local gateway |
| Server B | <SERVER_B_IP> | Aegis serve + relay handler (via Caddy port 80), node-b, target service (127.0.0.1:18081) |

**Connection path:**
```
Server A curl → http://<SERVER_B_IP>:80/__aegis/relay → Caddy → aegis relay handler → 127.0.0.1:18081
```
...
**Command:**
```bash
curl -s -X POST http://<SERVER_B_IP>:80/__aegis/relay \

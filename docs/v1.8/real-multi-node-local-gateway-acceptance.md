...
│              │     X-Aegis-Gateway-Token         │ <SERVER_B_IP> │
...
| B | Server B (<SERVER_B_IP>) | Relay target | public/private gateway | 80/443 |
| C | Server A (<SERVER_A_IP>) | Relay target | public/private gateway | 80/443 |
...
| Relay endpoint | http://<SERVER_B_IP>:<port>/__aegis/relay | ✅ |
...
```bash
ssh ubuntu@<SERVER_B_IP> "curl -H 'Host: api-c.example.com' http://127.0.0.1:<B_gateway_port>/health"
```
